...
mod tests {
    ...
    fn test_sort_assemble_sorters_by_name_with_dirs_first() {
        ...
        let meta_a = Meta::from_path(&path_a, &Flags::default()).expect("failed to get meta");

        // Create a dir;
        let path_z = tmp_dir.path().join("aaa");
        create_dir(&path_z).expect("failed to create dir");
        let meta_z = Meta::from_path(&path_z, &Flags::default()).expect("failed to get meta");
    ...
    fn test_sort_assemble_sorters_by_extension() {
        ...
        let meta_j = Meta::from_path(&path_j, &Flags::default()).expect("failed to get meta");

        // Create the file with txt extension;
        let path_t = tmp_dir.path().join("zzz.txt");
        File::create(&path_t).expect("failed to create file");
        let meta_t = Meta::from_path(&path_t, &Flags::default()).expect("failed to get meta");
    ...
    fn test_sort_assemble_sorters_by_version() {
        ...
        let meta_b = Meta::from_path(&path_b, &Flags::default()).expect("failed to get meta");

        let path_c = tmp_dir.path().join("12");
        File::create(&path_c).expect("failed to create file");
        let meta_c = Meta::from_path(&path_c, &Flags::default()).expect("failed to get meta");
    ...
    fn test_sort_assemble_sorters_no_sort() {
        ...
        let meta_d = Meta::from_path(&path_d, &Flags::default()).expect("failed to get meta");
